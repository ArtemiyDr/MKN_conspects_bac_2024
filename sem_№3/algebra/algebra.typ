...
#theorem[(Машке) 
    Пусть $k$ - поле, $G$ - конечная группа, $c h a r k divides.not |G|$, $V$ - $k[G]$-модуль, 
...
Везде далее $V$ - конечномерное векторное пространство над полем $K$ и $c h a r K divides.not |G|$
...
#lemma[ (Шура) 
    ...
    + $E n d_R (M)$ - тело (т.е. кольцо с делением).
] 
#proof[
    + Пусть $phi$ не изоморфизм, тогда $k e r (phi) " - модуль," eq.not {0}$, так как $M$ простой, то $k e r (phi) = M$, значит $phi$ - нулевой гомоморфизм. \ Если же $phi$ не нулевой, то $I m (phi) eq.not {0}$, значит $I m (phi) = N$, значит $phi$ - эпиморфизм, а так как $M$ простой, то $k e r (phi) = {0}$, значит $phi$ - изоморфизм.\
...
#proof[
    ...
    содержится в $a_h hat(K)_h$ единственным образом и не содержится в $a_g hat(K)_g$ для $g eq.not h$, значит $x$ содержится в линейной комбинации $sum_limits(g in I) a_g hat(K)_g$ единственным образом, значит если эта линейная комбинация нулевая, \ 
    то $a_g = 0 space forall g in I$, так как элементы $G$ - базис $k[G]$.    
]
