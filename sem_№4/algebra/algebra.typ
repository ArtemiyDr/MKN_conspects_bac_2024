...
#proof[
    Пусть $a|K| + b chi(1) = 1$, тогда $(a|K|chi(x_k))/(chi(1)) = (chi(x_k))/(chi(1)) - b chi(x_k)$ - алгебраическое целое, а тогда и $(chi(x_k))/(chi(1))$ тоже алгебраическое целое. Пусть теперь $alpha = |chi(x_k)|<chi(1)$, 
    $
      |N(alpha)| = |product_(sigma in G a l(QQ(zeta_n) slash QQ)) sigma alpha|<1 " так как корни переходят в корни"
    $
    Но тогда $N(alpha) = 0$, так как $N(alpha) in ZZ$, а тогда $alpha = 0$. 
]

#definition[
    Группа $G$ называется *простой*, если не содержит нетривиальных собсьвенных нормальных подгрупп.
]
...
#proof[
    Пусть $chi eq.not 1$ - неприводимый характер, $x_k eq.not 1$. Пусть $|chi(x_k)| = chi(1)$. Пусть $v in V_chi, g in G$ тогда $g x_k g^(-1) x_k^(-1) v = v$, но $G$ - простая, так что $G --> "Aut"_CC (V_chi)$ - вложение (иначе ядро дает нормальную подгруппу), значит $x_k in Z(G), space Z(G) = G$, противоречие с неабелевостью. Отсюда и предыдущей леммы: если $p divides.not chi(1) ==> space chi(x_k)$ = 0.
    $
...
#proof[ (теоремы Бернсайда)\
    ...
    + $G$ - не простая, тогда $N$ - нетривиальная свобственная нормальна подгруппа, по индукции $N, G slash N$ - разрешимы, тогда разреима и $G$ как произведение.
    + $G$ - простая, тогда выберем произвольную силовскую $p$-подгруппу $P$ для простого $p$, ее центр нетривиален. $K$ - класс сопряженности произвольного неединичного элемента $x_k$ в центре, тогда:
    $
...
#theorem[
    Пусть $x_1, dots, x_m$ и $u_1, ... u_n$ алгебраически независимы в $F$, и имеем 
    $
      K subset K(x_1, ..., x_m) subset F, space K subset K(u_1, ..., u_n) subset F - " первое расширение трансцендентно"
    $
...
#definition[
    Для конечно порожденного расширения $F$ над $K$ определим его *транcцендентную степень* как наименьшее число $n$, такое что $F slash K(x_1,...,x_n)$, где $x_1,...,x_n$ - *трансцендентный базис* - алгебраически независимые элементы в $F$.
]

#definition[
    Будем называть алгебру $A$ над полем $K$ *конечно порожденной* элементами $x_1, ..., x_n$, если $forall a in A, space e=exists f in K[t_1, ..., t_n]$ что $a = f(x_1, ..., x_n)$, обозначим $A = K[x_1, ..., x_n]$
]
...
#proof[
    ...
    + $A = K[x_1]$, тогда $y_1 = x_1$ - искомый элемент.
    ...
    $ \ Если бы $p_s in K$ то $A$ над $K[{x_i}_(i eq.not r+1)]$ было бы конечнопорожденным модулем, воспользовавшись индукционным предположением получим:$
      K subset K[y_1,...,y_r] subset K[{x_i}_(i eq.not r+1)] subset A
    $ Тогда $A$ над $K(y_1, ..., y_r)$ - конечно порожденный модуль.\ Значит $p_s$ - не константа, тогда сделаем замену $x_i^prime = x_i - x_(r+1)^(d^i)$ для некоторого $d, space 1<=i<=r$. Заметим что все еще $A = K[x_1^prime,...,x_r^prime, x_(r+1),...,x_n]$ и $x_(r+1)$ все также алгебраически зависит от $x_1,...,x_r:$ $
...
#corollary[
    Все максимальные имдеалы в $L = K[t_1, ..., t_n]$, где $K$ - алгебраически замкнутое поле, имеют вид $(t_1 - alpha_1, ..., t_n - alpha_n)$, где $alpha_i in K$
]
#proof[
   ...
   + $<==$: Пусть $I$ - максимальный идеал, тогда $L slash I$ - поле, а тогда по предыдущему следствию $L slash I$ - конечно порожденное расширение над $K$, тогда найдется $alpha_1,...,alpha_n in K$ такие что $L slash I = K(alpha_1,...,alpha_n)$, тогда $I supset (t_1 - alpha_1, ..., t_n - alpha_n)$ и они совпадают по максимальности.
]

#lemma[(частный случай теоремы Гильберта о нулях для $n=1$)\
    Для $L = K[t_1,...,t_n], space f_1,...,f_m in L$:
    $
