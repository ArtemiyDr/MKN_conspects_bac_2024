...
#proof[
    Пусть $p in U inter "per"f, space m$ - период $p$. Рассмотрим прямую, проходящую через эту точку, порожденную собственным ветором числа $1/lambda = (3+sqrt(5)) /2$ линейного отобьражения $F$. Выберем на прямой отрезочек длины $delta$, содержащий $p$. Рассмотрим образ этого отрезка, его длина $delta dot 1/lambda$, проделаем это $m$ раз, получим длину $delta dot 1/(lambda^m)$ - причем $p$ перейдет в $p$. Сделаем так еще много раз, по иррациональности $1/lambda$ получим сколь угодно плотное покрытие отрезка при факторизации, в тогда по теореме имеем топологическую транзитивность, а из нее топологическое перемешивание. 
]

#example[\
    *Подкова Смейла*:\
    Рассмотрим отображение $f_1:[0,1]^2 --> [0,1/4] times [0,4]$ - простое сжатие по одной координате и растяжение по другой, а также $f_2$, которая отображает $[0,1/4] times [0,4]$ в "подкову", заходящую "ножками" на квадрат (далее $Q$) так, что в пересечении дает два прямоугольника - $Q_0, Q_1 space (f = f_1 compose f_2)$:
    $
    ...
    $
    Легко видеть, что $x$ будет принадлежать $Lambda$ тогда и только тогда, когда он принадлежит счетному количеству вертикальных вложенных прямоугольничков увеличивающихся рангов, или горизонтальных убывающих грангов с $f^(-1)$, а тогда $Lambda$ суть есть объединение вертикальных и горизонтальных отрезочков, получаемых при счетном пересечении всех вложенных прямоугольников возрастающих по модулю рангов. Получаем по каждой оси почти стандартное канторово множество (делим на пять частей, выкидываем нечетные) и $Lambda$ - произведение двух таких канторовых множеств.\
    #align(center)[(без картинок, увы)]
]

#theorem[ (Смейла)\
    Существует гомоморфизм $h: Lambda --> Pi$, где $Pi$ - пространство двоичных последовательностей для сдвига бернулли: $h compose f|_Lambda = sigma compose h|_Lambda$
]
#proof[
    ...
    Проверим биективность: очевидно по построению, последовательность ${a_k}_(k in ZZ)$ определяет одновременно последовательность вертикальных вложенных прямоугольничков и горизонтальных, то есть одновременно определяет горизонтальный и вертикальный отрезок, что в пересечении дает одну точку, из чего следует однозначность обратного отображения.
]
